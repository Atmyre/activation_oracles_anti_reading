...
def find_pattern_in_tokens(
    token_ids, special_token_str: str, num_positions: int, tokenizer: AutoTokenizer
) -> list[int]:
    # Normalize: transformers 5.x apply_chat_template may return BatchEncoding
...
def create_training_datapoint(
    ...
    def _coerce_ids(x):
        ...
        if hasattr(x, "tolist"):
            x = x.tolist()
        return list(x)
    def _apply_template(messages, add_gen):
        try:
            return tokenizer.apply_chat_template(
                messages, tokenize=True, add_generation_prompt=add_gen, return_tensors=None, padding=False, enable_thinking=False
            )
        except TypeError:
            return tokenizer.apply_chat_template(
                messages, tokenize=True, add_generation_prompt=add_gen, return_tensors=None, padding=False
            )
    input_prompt_ids = _coerce_ids(_apply_template(input_messages, add_gen=True))
    full_messages = input_messages + [{"role": "assistant", "content": target_response}]
    full_prompt_ids = _coerce_ids(_apply_template(full_messages, add_gen=False))
